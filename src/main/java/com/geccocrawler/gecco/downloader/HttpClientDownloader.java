...
import com.geccocrawler.gecco.utils.ContentTypeUtils;
...
public class HttpClientDownloader extends AbstractDownloader {
	...
	public HttpClientDownloader() {
		...
		httpClient = HttpClientBuilder.create().setDefaultRequestConfig(clientConfig)
				.setConnectionManager(syncConnectionManager).setRetryHandler(new HttpRequestRetryHandler() {
					@Override
	...
	public HttpResponse download(HttpRequest request, int timeout) throws DownloadException {
		...
		try {
			...
			if (status == 302 || status == 301) {
				...
				if (ContentTypeUtils.isText(contentType)) {
